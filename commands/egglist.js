...
module.exports.run = async (bot, message, args) => {
  ...
  db.all(`SELECT * FROM Users WHERE Tag = '${message.author.id}'`, (err, items) => {
    const eggs = parseInt(items[0].Gems);
    const beginneregg = (eggs >= 0) ? ':unlock:' : ':lock:';
    const spottedegg = (eggs >= 25) ? ':unclok:' : ':lock:';
    ...
    .setColor('#9c13f7')
    .setDescription(`${beginneregg} - <:Beginner_Egg:592440252979871745> Beginner Egg - 0 eggs\n${spottedegg} - <:Spotted_Egg:592446342228606976> Spotted Egg - 25 eggs`)
